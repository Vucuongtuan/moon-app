...
export function useSearchQuery(keyword: string, options: UseSearchQueryOptions = {}) {
  ...
  const { data, isLoading, isError, isFetching, refetch } = useQuery({
    ...
    queryFn: () => findSearchByKeyword(keyword, locale),
    // enabled: !!debouncedQuery && (options.enabled ?? true),
    // staleTime: 1000 * 60 * 5, // 5 minutes
  });
  console.log({data,isLoading,isError,isFetching,locale})
  return {
    data: data?.docs || [],
    meta: {
